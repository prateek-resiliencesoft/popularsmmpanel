          ...
          <div class="card-header d-flex align-items-center">
            <div class="tabs-list">
              <ul class="nav nav-tabs">
                <?php
                  if (!get_option("is_active_paypal") && !get_option("is_active_stripe") && !get_option("is_active_2checkout") && !get_option("is_active_manual") && !get_option("is_active_perfect_money")) {
                ?>
                <li class="m-t-10">
                ...
                <?php }?>
                  <?php
                  if (get_option("is_active_perfect_money")) {
                      ?>
                      <li class="m-t-10">
                          <a data-toggle="tab" href="#perfectmoney"><i class="fa fa-credit-card"></i> <?=lang("perfectmoney")?></a>
                      </li>
                  <?php }?>
              
                <?php
                    $payments_method = get_payments_method();
//                print_r($payments_method);
            ...
            <div class="tab-content">
              ...
              if (get_option("is_active_perfect_money")) {
                  ...
                  <div id="perfectmoney" class="tab-pane fade">
                      <form class="form actionForm" action="<?=cn($module."/process")?>" data-redirect="<?=cn($module."/perfectmoney")?>" method="POST">
                          <div class="row">
                              <div class="col-md-12">
                                  <?php
                                  if (get_option("perfect_money_account_id", '') != "" && get_option("perfect_money_account_name", '') != "" ) {
                                      ?>
                                      <div class="for-group text-center">
                                          <img src="<?=BASE?>/assets/images/pm.png" width="200px" height="100px"  alt="perfect money icon">
                                          <p class="p-t-10"><small><?=sprintf(lang("you_can_deposit_funds_with_paypal_they_will_be_automaticly_added_into_your_account"), 'perfectmoney')?></small></p>
                                      </div>

                                      <div class="form-group">
                                          <label><?=sprintf(lang("amount_usd"), get_option("currency_code",'USD'))?></label>
                                          <input class="form-control square" type="number" name="amount" placeholder="<?=get_option('payment_transaction_min')?>" id="">
                                          <input type="hidden" name="payment_method" value="perfect_money">
                                      </div>


                                      <div class="form-group">
                                          <small class=""><?=lang("transaction_fee")?>: <strong><?=(get_option("perfect_money_chagre_fee", 4))?>%</strong></small>
                                      </div>
